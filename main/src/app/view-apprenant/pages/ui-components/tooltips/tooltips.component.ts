...
import type { ApprenantCourDto } from 'src/app/models/ApprenantCour';
  ...
  export class AppTooltipsComponent implements OnInit{
          ...
          switchMap((user: any) => {
            ...
            return forkJoin([
              ...
              switchMap(([apprenantCourses, souhaitsList]) => {
                const matchedCourse = apprenantCourses?.find((apprenantCour :ApprenantCourDto) =>apprenantCour.courId === this.courId && apprenantCour.apprenantId === this.apprenantId)
                console.log('MatchedCourse:', matchedCourse);
                this.matchedCourse = matchedCourse;
                this.isCourseAdded = !!matchedCourse;
    ...
    addApprenantCour(courId: number): void {
      const apprenantCour:ApprenantCourDto = {
        apprenantCourid: 0,
      ...
      this.apprenantCourService.addApprenantCour(apprenantCour).subscribe(
        (response) => {
          console.log('Course added to ApprenantCour:', response);
    ...
    deleteSouhait(): void {
      ...
      console.log('MatchedSouhait:', this.matchedSouhait); 
